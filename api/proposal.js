...
<div class="q-slide"><div class="q-slide-container"><p class="slide-eyebrow">Investment &amp; Next Steps</p><div class="investment-block"><h2 class="slide-title">Commitment</h2><table class="investment-table">${investmentRows}<tr class="total-row"><td>Total</td><td>${e(d.investmentTotal)}</td></tr>${paymentStructureRow}</table>${paymentNoteBlock}</div><div class="investment-block"><h2 class="slide-title">Next Steps</h2><ul class="next-steps-list">${nextStepsItems}</ul></div></div></div>
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (mode === 'approve') {
                const isTechnicalApproval = data.projectType === 'technical';

                // ── TECHNICAL / SYSTEMS — no intake form, no code ──────────
                if (isTechnicalApproval) {
                    // Notify Alberto
                    await resend.emails.send({
                        from: 'Moood Studio <notifications@moood.studio>',
                        to: ['alberto.contreras@gmail.com'],
                        subject: `[APPROVED] ${data.projectName} — ${data.clientName}`,
                        html: buildEmail({
                            projectName: data.projectName,
                            context: 'Project Approved',
                            headline: `${data.projectName}<br>is confirmed.`,
                            body: `Approved by ${data.clientName}. Technical engagement — no intake form required. Reach out directly to align on kickoff.`,
                            signature: false,
                        }),
                    });

                    // Notify Client — simple confirmation, no code, no form
                    if (data.clientEmail) {
                        await resend.emails.send({
                            from: 'Alberto at Moood.Studio <alberto@moood.studio>',
                            to: [data.clientEmail],
                            subject: `You're all set — ${data.projectName}`,
                            html: buildEmail({
                                projectName: data.projectName,
                                ...
                                body: [
                                    ...
                                    `We'll be in touch within 24 hours to align on kickoff. No forms, no waiting — just work.`
                                ],
                            }),
                        });
                    }
                ...
                // ── STANDARD FLOW — brand/web projects with intake form ────
